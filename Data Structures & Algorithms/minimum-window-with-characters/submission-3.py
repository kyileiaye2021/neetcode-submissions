class Solution:
    def minWindow(self, s: str, t: str) -> str:
        ...
        # max window size = 0
        # iterate thru s
        # while the curr char is not in the t
        #   move both l and r
        # decrement the freq count of curr ele in t
        # curr_window size = r - l + 1
        ...
        for r in range(len(s)):
            ...
            while have == need:
                # update the min window and min substr
                if (r - l + 1) < res_len:
                    res_len = (r - l + 1)
                    res_substr = s[l: r + 1]
                
                # take out left pointer ele
                s_map[s[l]] -= 1

                # update the have if the freq of ele is not the same as t_map[ele]
        ...
        return res_substr if res_len != float('inf') else ""
